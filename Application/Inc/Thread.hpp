...
class Thread {
  public:
    ...
    // your task‑wrapper
    template<void(Thread::*F)()> static void task(void* arg) {
        (static_cast<Thread*>(arg)->*F)();
    }

    // all your methods...
    void init();            TaskHandle_t init_handle;
    void sendTX();          TaskHandle_t sendTX_handle;
    void parseRX();         TaskHandle_t parseRX_handle;
    void telemetry();       TaskHandle_t telemetry_handle;
    void runner();          TaskHandle_t runner_handle;
    void flashSave();       TaskHandle_t flashSave_handle;
    void flashLoad();       TaskHandle_t flashLoad_handle;
    void calculator();      TaskHandle_t calculator_handle;
    void dacUpdate();       TaskHandle_t dacUpdate_handle;
    void schedule_20Hz();   TaskHandle_t schedule_20Hz_handle;
    void watchdog();        TaskHandle_t watchdog_handle;
    ...
    // now these fields magically carry the right Entry/Transition/Anonymous types:
